...
#game window
tile_size = 30
world_size = 20  # Fixed size for the world data
cols = 20        # Window size (can be different from world_size)
margin = 80
screen_width = tile_size * world_size  # Use world_size instead of cols
screen_height = (tile_size * world_size) + margin
...
dirt_img = pygame.image.load('img/floor.png')
grass_img = pygame.image.load('img/carpet.png')
...
for tile in range(cols):
    world_data[cols - 1][tile] = 2  # Bottom boundary
    world_data[0][tile] = 1  # Top boundary
    world_data[tile][0] = 1  # Left boundary
    world_data[tile][cols - 1] = 1  # Right boundary
...
def draw_grid():
    for c in range(world_size + 1):  # Use world_size instead of cols
